const mySecret = process.env['ADMIN']
...
io.on('connection', (socket) => {
	socket.on('join', (data) => {
		...
		if (!Object.keys(allsockets).includes(room)) {
			...
			socket.emit('message', {
				name: 'server',
				message: `Welcome to LowChat v3! You are in room "${socket.proto.room}". To join a different room, type /join [room]. Type /help to see more commands and use /nick [name] to change your name`
			});
